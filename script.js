...
const UI = (() => {
  ...
  function renderDashboard() {
    ...
    // KPIs
    _setText('kpi-salary', _fmt(salary));
    _setText('kpi-balance', _fmt(balance));
    _setText('kpi-balance-pct', `${Math.max(0, (100 - pct)).toFixed(1)}% disponível`);
    _setText('kpi-spent', _fmt(total));
    _setText('kpi-spent-pct', `${pct.toFixed(1)}% do salário`);
    _setText('kpi-txcount', filtered.length.toString());
    _setText('kpi-txperiod', 'Este mês');
    ...
    if (bar) {
      if (pct > 90) bar.style.background = 'linear-gradient(90deg, #FF3B3B, #FF7A7A)';
  ...
  function _renderInsight(pct, expenses) {
    ...
    let alert = null;
    cats.forEach(c => {
      if (!alert && c.limit > 0 && salary > 0) {
        const spent = catSpend[c.id] || 0;
        const catPct = spent / salary * 100;
        if (catPct > c.limit) {
          alert = `⚠️ Você ultrapassou o limite de <strong>${c.name}</strong> (${catPct.toFixed(1)}% vs limite de ${c.limit}%).`;
        }
      }
    });
    ...
    if (salary === 0) {
      msg = 'Configure seu salário nas <strong>Configurações</strong> para ativar o assistente inteligente.';
      icon = '⚙️';
    } else if (expenses.length === 0) {
      msg = 'Nenhum gasto registrado este mês. Comece a registrar para receber insights personalizados.';
      icon = '📝';
    } else if (alert) {
      msg = alert;
      icon = '🚨';
      insightCard.style.borderLeftColor = '#FFD600';
    } else if (pct > 95) {
      msg = `Atenção: você gastou <strong>${pct.toFixed(1)}%</strong> do seu salário. Saldo crítico de ${_fmt(State.getSalary() - expenses.reduce((s,e)=>s+e.value,0))}.`;
      icon = '🚨';
    } else if (pct > 80) {
      msg = `Você gastou <strong>${pct.toFixed(1)}%</strong> do seu salário. Controle os gastos para não ultrapassar o limite.`;
      icon = '⚠️';
    } else if (pct > 60) {
      msg = `Você utilizou ${pct.toFixed(1)}% do salário. Ainda há <strong>${_fmt(State.getSalary() - expenses.reduce((s,e)=>s+e.value,0))}</strong> disponíveis este mês.`;
      icon = '📊';
      insightCard.style.borderLeftColor = '#FFD600';
    } else if (pct > 0) {
      msg = `Ótimo controle! Você gastou apenas <strong>${pct.toFixed(1)}%</strong> do salário. Considere investir o restante.`;
      icon = '✅';
      insightCard.style.borderLeftColor = '#00E676';
    }
